		...
		<view class="container">
			<view class="form">
				...
				<u-input :placeholder="$t('registerUser.mobileCode')" border="none" clearable type="number"
					...
					<view slot="suffix">
						<!-- <u-button @tap="getCode" type="success" :customStyle="{
													color:'#fff',
													fontFamily: 'Arial',
													height: '60rpx',
													border: 'none',
													background: 'rgb(75 78 93)'
												}">
							{{tips}}
						</u-button> -->
				...
				<view class="save_forget">
					<view class="save">
						...
						<view class="text" @click="remoberPassword">I agree laga</view>
					</view>
				...
				<wyb-button :class="loginBtnEnable?'loginBtn':'disabledLogin'" type="hollow" :ripple="true"
					:disabled="loginBtnEnable?false:true" @click="login">Register</wyb-button>
				<!-- logo -->
	...
	export default {
		...
		computed: {
			...
			loginBtnEnable() {
				...
				return (userId&&password&&confirm&&invitationCode&&name&&email&&mobile&&agree)? true : false;
			}
		},
		methods: {
			close() {
			...
			handleProtocol() {
				if (this.data) {
					...
					this.GetBaseLanguageSetData(true);
				}
			}
		},
		onLoad(options) {
