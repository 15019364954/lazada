	...
	<view>
		...
		<view class="container" :style="{paddingTop: isPhone?statusBarHeight+'px':60+'px'}">
			<view class="form">
				...
				<!-- 输入手机号码 -->
				<view class="inputTitle phoneInputTitle">{{$t('withdrawalPassword.currentBindMobile')}}</view>
				<u-input border="none" clearable :disabled="true" type="text" v-model="mobile" :class="phoneFocusClass"
					placeholderClass="placeholderClass" @focus="InputFocus('phone')" @blur="InputBlur()">
				</u-input>

				<view class="inputTitle phoneInputTitle">{{$t('withdrawalPassword.enterMobileVerCode')}}</view>
				...
				<!-- 输入密码 -->
				<view class="inputTitle phoneInputTitle">{{$t('withdrawalPassword.setPwd')}}</view>
				...
				<!-- 确认密码 -->
				<view class="inputTitle phoneInputTitle">{{$t('withdrawalPassword.confirmPwd')}}</view>
				<u-input v-if="plaintext2" border="none" type="text" clearable v-model="form.confirm"
		...
		<u-code :seconds="seconds" ref="uCode" @change="codeChange" :startText="$t('withdrawalPassword.startText')"
			changeText="XS" :endText="$t('withdrawalPassword.Refresh')">
		</u-code>
	...
	export default {
		...
		methods: {
				...
				if (this.$refs.uCode.canGetCode) {
					...
					if (!this.mobile) {
						this.$refs.uToast.show({
							message: this.$t('withdrawalPassword.pleaceBindMobile'),
							type: 'error',
							duration: 2000,
						})
						return;
					...
					uni.showLoading({
						title: this.$t('withdrawalPassword.sending')
					})
					...
					if (res.data.resultCode == 1) {
						...
						_this.$refs.uToast.show({
							message: _this.$t('withdrawalPassword.sendsuccess'),
							type: 'success',
							duration: 2000,
						})
					} else {
			...
			rules() {
				...
				if (!this.mobile) { //空
					return this.$t('withdrawalPassword.mobileIsEmpty');
				}
				if (this.mobile.length < 5 || this.mobile.length > 20) { //少于5大于20
					return this.$t('withdrawalPassword.mobileRange');
				}
				if (!authCode) { //空
					return this.$t('withdrawalPassword.mobileCodeIsEmpty');
				}
				if (authCode.length != 4) { //少于5
					return this.$t('withdrawalPassword.mobileCodeRange');
				}
				if (!newPassword) { //空
					return this.$t('withdrawalPassword.pwdIsEmpty');
				}
				if (newPassword.length < 6 || newPassword.length > 12) { //少于6大于12
					return this.$t('withdrawalPassword.pwdRange');
				}
				if (!confirm) { //空
					return this.$t('withdrawalPassword.EnterConfirmpassword');
				}
			...
			async comfrim() {
				...
				if (newPassword !== confirm) {
					this.$refs.uToast.show({
						message: this.$t('withdrawalPassword.pwdDifferent'),
						type: 'error',
						duration: 2000,
					})
					return;
				...
				const params = {
					newPassword: md5(this.form.newPassword), //密码
					mobile: this.mobile,
					authCode: this.form.authCode, //手机验证码
					userID: uni.getStorageSync('userInfo').userID
				}
				uni.showLoading();
				const res = await ResetDPassword(params);
				uni.hideLoading();
				const networkError = this.$u.utils.handleNetwork(res);
				if (networkError) {
					this.$refs.uToast.show({
				...
				if (res.data.resultCode == 1) {
					//修改成功跳转到个人中心
					this.$refs.uToast.show({
						message: this.$t('withdrawalPassword.edit'),
						type: 'success',
						duration: 2000,
						complete: () => {
							uni.switchTab({
